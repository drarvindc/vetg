...
<body>
    <div class="container mt-5">
        ...
        <form id="treatmentForm">
            ...
            <button type="button" class="btn btn-danger ms-2" id="deleteTreatmentBtn" style="display:none;">Delete Treatment</button>
    ...
    <script>
        ...
        document.addEventListener('click', (e) => {
                        ...
                        if (data.success) {
                            ...
                            if (plan) {
                                ...
                                plan.steps.forEach((step, index) => {
                                    if (!step.treatment_name) return; // Skip invalid steps
            ...
            if (e.target.classList.contains('deletePlan')) {
                if (confirm('Are you sure you want to delete this plan?')) {
                    ...
                    fetch('/backend/plans/delete_plan.php', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => response.text())
                    .then(text => {
                        console.log('Delete plan raw response:', text);
                        let data;
                        try {
                            data = JSON.parse(text);
                        } catch (parseError) {
                        ...
                        console.log('Delete plan parsed response:', data);
            ...
            if (e.target.classList.contains('deleteTreatmentBtn')) {
                if (confirm('Are you sure you want to delete this treatment?')) {
                    const formData = new FormData();
                    formData.append('step_id', document.getElementById('treatment_step_id').value);
                    fetch('/backend/plans/delete_treatment.php', {
                        method: 'POST',
                        body: formData
                    })
        ...
        document.getElementById('planForm').addEventListener('submit', async (e) => {
            ...
            const steps = Array.from(formData.entries()).filter(([key]) => key.startsWith('steps['));
            for (let [key, value] of steps) {
                if (key.includes('[treatment_name]') && !value.trim()) {
                    document.getElementById('responseArea').className = 'alert alert-danger';
                    document.getElementById('responseArea').textContent = 'Treatment name cannot be empty';
                    document.getElementById('responseArea').style.display = 'block';
                    return;
                }
            }
        ...
        document.getElementById('treatmentForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            const treatmentName = formData.get('treatment_name').trim();
            if (!treatmentName) {
                document.getElementById('treatmentResponseArea').className = 'alert alert-danger';
                document.getElementById('treatmentResponseArea').textContent = 'Treatment name cannot be empty';
                document.getElementById('treatmentResponseArea').style.display = 'block';
                return;
            }
            const responseArea = document.getElementById('treatmentResponseArea');
                    ...
                    data.plans.forEach(plan => {
                        if (plan.plan_id) { // Only show actual plans
                            ...
                            row.innerHTML = `
                                ...
                                <td>${plan.steps.map(step => step.treatment_name ? `${step.treatment_name} (${step.type}, ${step.spacing_days} days${step.duration_months ? `, ${step.duration_months} months` : ''}, ${step.species_tags || 'All'})` : '').filter(Boolean).join('<br>')}</td>
                                <td>
                                    <button class="btn btn-sm btn-primary editPlan" data-plan-id="${plan.plan_id}">Edit</button>
                                    <button class="btn btn-sm btn-danger deletePlan" data-plan-id="${plan.plan_id}">Delete</button>
                                </td>
                            `;
        ...
        async function loadTreatments() {
            try {
                ...
                if (data.success) {
                    data.plans.forEach(plan => {
                        if (!plan.plan_id) { // Show standalone treatments
                            plan.steps.forEach(treatment => {
                                if (!treatment.treatment_name) return; // Skip invalid treatments
                                const row = document.createElement('tr');
                                row.innerHTML = `
                                    <td>${treatment.type || 'N/A'}</td>
                                    <td>${treatment.treatment_name || 'N/A'}</td>
